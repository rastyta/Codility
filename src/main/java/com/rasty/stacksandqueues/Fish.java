...
//https://app.codility.com/demo/results/trainingGXV34M-G2G/
/**
 ...
 *
 * <p>Fish number P is represented by A[P] and B[P]. Array A contains the sizes of the fish. All its
 * elements are unique. Array B contains the directions of the fish. It contains only 0s and/or 1s,
 * where:
 *
 * <p>0 represents a fish flowing upstream, 1 represents a fish flowing downstream. If two fish move
 * in opposite directions and there are no other (living) fish between them, they will eventually
 * meet each other. Then only one fish can stay alive − the larger fish eats the smaller one. More
 * precisely, we say that two fish P and Q meet each other when P < Q, B[P] = 1 and B[Q] = 0, and
 ...
 * eats P, and Q will still be flowing upstream. We assume that all the fish are flowing at the same
 * speed. That is, fish moving in the same direction never meet. The goal is to calculate the number
 ...
 *
 * <p>that, given two non-empty arrays A and B consisting of N integers, returns the number of fish
